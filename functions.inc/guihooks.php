...
function userman_configpageload() {
	...
	if ($action != 'del') {
		...
		if($extdisplay != '') {
			...
			$currentcomponent->addjsfunc('usermanPassword()',"if(\$('#userman\\\|assign').val() != 'add') {\$('#userman\\\|password').attr('disabled',true);if($('#userman\\\|username_cb').prop('checked')) { $('#userman\\\|username_cb').click() }\$('#userman\\\|username_cb').attr('disabled',true);} else {\$('#userman\\\|password').attr('disabled',false);\$('#userman\\\|username_cb').attr('disabled',false)}");
			$currentcomponent->addguielem($section, new gui_textbox_check('userman|username','', _('Username'), _('If Create New User is selected this will be the username. If blank the username will be the same number as this device'),'','',false,0,true,_('Use Custom Username'),""));
		...
		case "add":
			...
			if(isset($_REQUEST['userman|assign']) && !empty($extension)) {
				if($_REQUEST['userman|assign'] == 'add') {
					$username = (!empty($_REQUEST['userman|username_cb']) && !empty($_REQUEST['userman|username'])) ? $_REQUEST['userman|username'] : $extension;
					$ret = $userman->addUser($username, $_REQUEST['userman|password'], $extension, _('Autogenerated user on new device creation'));
					if($ret['status']) {
		...
		case "edit":
			if(isset($_REQUEST['userman|assign']) && $_REQUEST['userman|assign'] == 'add') {
				...
				if(!empty($userO['id']) && ($userO['id'] != $_REQUEST['userman|assign'])) {
					...
					//run this last so that hooks to other modules get the correct information
					$username = (!empty($_REQUEST['userman|username_cb']) && !empty($_REQUEST['userman|username'])) ? $_REQUEST['userman|username'] : $userO['username'];
					$userman->updateUser($userO['username'],$username,'none');

					$ret = $userman->getUserByUsername($username);
