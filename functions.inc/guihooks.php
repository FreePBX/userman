...
$js = <<<JS
	...
	$("#userman_group_submitted").prop("disabled",true);
...
$js = <<<JS
	...
	if($("#userman_assign").val() == "add") {
		...
		if(permissions.modifyGroup) {
			...
			$("#userman_group_submitted").prop("disabled",false);
		}
		$("#userman_group").trigger("chosen:updated");
	} else {
		$("#userman_username").prop("disabled",true);
		$("#userman_username_cb").prop("disabled",true);
		$("#userman_password").prop("disabled",true);
		// compare as strings: the option value is a string while the ids in the
		// ajax data are numeric, so a bare indexOf never matches
		var assignVal = String($('#userman_assign').val());
		var selected = [];
		$.each(groups, function(k,v) {
			if((v.users || []).map(String).indexOf(assignVal) > -1) {
				selected.push(String(v.id));
			}
		})
		$("#userman_group").val(selected);
		if(!permissions.modifyGroup || $("#userman_assign").val() == "none") {
			$("#userman_group").prop("disabled",true);
			$("#userman_group_submitted").prop("disabled",true);
			$("#userman_group").trigger("chosen:updated");
		} else {
...
JS;
		...
		if($extdisplay != '') {
			...
			$currentcomponent->addguielem($section, new gui_textbox('userman_password',md5(uniqid()), _('Password For New User'), _('If Create New User is selected this will be the autogenerated users new password'),'','',false,0,$passDisable,false,'password-meter confidential',false),$category);
			$groupsDisabled = !$permissions['modifyGroup'];
			$currentcomponent->addguielem($section, new gui_multiselectbox('userman_group', $allGroups, $groups, _('Groups'), _('Groups that this user is a part of. You can add and remove this user from groups in this view as well'), false, '',$groupsDisabled,"chosenmultiselect"),$category);
			// Marker so the process side can tell "field was editable" apart from
			// "field was disabled/absent"; a bare empty multiselect posts nothing
			if(!$groupsDisabled) {
				$currentcomponent->addguielem($section, new gui_hidden('userman_group_submitted', '1'),$category);
			}
		} else {
			//New Extension
			$selarray = [["value" => 'none', "text" => _('None')]];
...
/**
 * Sync a user's group membership from the posted 'userman_group' field.
 *
 * The Groups multiselect only reaches $_POST when it was rendered enabled, and a
 * disabled or absent field is indistinguishable from "deselect everything". The
 * hidden 'userman_group_submitted' marker (kept enabled/disabled in lock-step with
 * the multiselect) tells us the admin could actually see and edit the field; without
 * it group membership must be left untouched instead of being wiped.
 *
 * @param object $userman The Userman BMO object
 * @param int    $uid     The user to sync
 * @param int    $dirid   The directory whose groups may be modified
 */
function userman_sync_groups_from_post($userman, $uid, $dirid) {
	if(empty($_POST['userman_group_submitted'])) {
		return;
	}
	$usettings = $userman->getAuthAllPermissions($dirid);
	if(empty($usettings['modifyGroup'])) {
		return;
	}
	$posted = isset($_POST['userman_group']) && is_array($_POST['userman_group']) ? $_POST['userman_group'] : [];
	foreach($userman->getAllGroups($dirid) as $group) {
		if(in_array($group['id'],$posted) && !in_array($uid,$group['users'])) {
			$group['users'][] = $uid;
			$userman->updateGroup($group['id'],$group['groupname'], $group['groupname'], $group['description'], $group['users']);
		} elseif(!in_array($group['id'],$posted) && in_array($uid,$group['users'])) {
			$group['users'] = array_diff($group['users'], [$uid]);
			$userman->updateGroup($group['id'],$group['groupname'], $group['groupname'], $group['description'], $group['users']);
		}
	}
}
		...
		case "add":
			...
			if(isset($_REQUEST['userman_assign']) && !empty($extension)) {
				if($_REQUEST['userman_assign'] == 'add') {
					...
					if($ret['status']) {
						userman_sync_groups_from_post($userman, $ret['id'], $directory);
						if(!empty($email)) {
					...
					if(!empty($ret) && $o['status']) {
						userman_sync_groups_from_post($userman, $ret['id'], $ret['auth']);
					} elseif(!$o['status']) {
						echo "<script>alert('".$ret['message']."')</script>";
						return false;
					}
