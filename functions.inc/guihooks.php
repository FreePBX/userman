...
function userman_configpageload() {
	...
	if ($action != 'del') {
		...
		if($extdisplay != '') {
			...
			$currentcomponent->addjsfunc('usermanPassword()',"if(\$('#userman_assign').val() != 'add') {\$('#userman_password').attr('disabled',true);if($('#userman_username_cb').prop('checked')) { $('#userman_username_cb').click() }\$('#userman_username_cb').attr('disabled',true);} else {if($('#userman_assign option[value=\"".$extdisplay."\"]').length == 0){\$('#userman_username_cb').click();}\$('#userman_password').attr('disabled',false);\$('#userman_username_cb').attr('disabled',false)}");
			...
			$currentcomponent->addjsfunc('usermanUsername()',"if(\$('#userman_username_cb').prop('checked')) {var users = ".json_encode($uUsers)."; if(isEmpty(\$('#userman_username').val()) || users.indexOf(\$('#userman_username').val()) >= 0) {return true;}} return false;");
			$currentcomponent->addjsfunc('usermanPassword()',"if(\$('#userman_assign').val() != 'add') {\$('#userman_password').attr('disabled',true);if($('#userman_username_cb').prop('checked')) { $('#userman_username_cb').click() }\$('#userman_username_cb').attr('disabled',true);} else {\$('#userman_password').attr('disabled',false);\$('#userman_username_cb').attr('disabled',false)}");
		...
		case "add":
			...
			if(isset($_REQUEST['userman_assign']) && !empty($extension)) {
				if($_REQUEST['userman_assign'] == 'add') {
					...
					$user = $userman->getUserByID($_REQUEST['userman_assign']);
					//run this last so that hooks to other modules get the correct information
					$userman->updateUser($user['id'],$user['username'],$user['username'],$extension);
				}
			}
		break;
		case "edit":
			if(isset($_REQUEST['userman_assign']) && $_REQUEST['userman_assign'] == 'add') {
				...
				if(!empty($userO['id']) && ($userO['id'] != $_REQUEST['userman_assign'])) {
					...
					$user = $userman->getUserByID($_REQUEST['userman_assign']);
					//run this last so that hooks to other modules get the correct information
					$ret = $userman->updateUser($user['id'], $user['username'],$user['username'],$extension);
				}
