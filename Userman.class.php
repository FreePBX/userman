...
class Userman extends \FreePBX_Helpers implements \BMO {
	...
	public function processQuickCreate($tech, $extension, $data) {
		if(isset($data['um']) && $data['um'] == "yes") {
			$pass = md5(uniqid());
			$directory = $this->getDefaultDirectory();
			$ret = $this->addUser($directory['id'], $extension, $pass, $extension, _('Autogenerated user on new device creation'), array('email' => $data['email'], 'displayname' => $data['name']));
			if($ret['status']) {
		...
		if(isset($_POST['submittype'])) {
			switch($_POST['type']) {
				...
				case 'group':
					...
					if($request['group'] == "") {
						$ret = $this->addGroup($directory, $groupname, $description, $users);
						if($ret['status']) {
							$this->message = array(
								'message' => $ret['message'],
								'type' => $ret['type']
				...
				case 'user':
					...
					if($request['user'] == "") {
						$ret = $this->addUser($directory, $username, $password, $default, $description, $extraData);
						if($ret['status']) {
