...
class Userman extends FreePBX_Helpers implements BMO {
	...
	*/
	...
	public function generateTemplateCreatorUser(){
		$directory = $this->getDefaultDirectory();
		$ret = $this->addUserByDirectory($directory['id'], 'FreePBXUCPTemplateCreator', md5('1a2b3c@fd48jshs03123ld'), 'TemplateCreator', _('Autogenerated user For Template Creation'), array('email' => '', 'displayname' => 'Template Creator'));
		if($ret['status']) {
			...
			$sql ="select id from userman_users where username ='FreePBXUCPTemplateCreator'";
			$sth = $this->db->prepare($sql);
			$sth->execute();
			$result = $sth->fetch(PDO::FETCH_ASSOC);
			if($result['id']){
	...
	public function getUnlockKeyTemplateCreator(){
		$uid = $this->generateTemplateCreatorUser();
