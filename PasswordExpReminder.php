    ...
    public function checkPasswordReminder($request)
    {
        ...
        if (\FreePBX::Modules()->checkStatus('pbxsaml') && \FreePBX::Pbxsaml()->isLicensed()) {
            ...
            if(!empty($getUser)){
                ...
                // check saml table has the entry for this user
                $isSamlUser = \FreePBX::Pbxsaml()->isSamlUser($getUser['id']);
                if($usamlenabled && \FreePBX::Pbxsaml()->isLicensed() && $isSamlUser){
                    return [
                        'loginfailed' => true,
                        'status'=>false,
                        'samlenabled'=>$usamlenabled,
